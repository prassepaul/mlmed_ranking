...
def dense_attention_layer(inputs, return_alphas=False, name=None):
    ...
    with tf.compat.v1.variable_scope(
        ...
        alphas = tf.compat.v1.layers.dense(
            inputs, feature_size,
...
def embedding_layer(inputs, vocab_size, embed_size, name=None):
    ...
    with tf.compat.v1.variable_scope(
        ...
        embedding_matrix = tf.compat.v1.get_variable(
            'embedding_matrix',
            initializer=tf.random.normal((vocab_size, embed_size)),
            trainable=True
        )
        """
        TODO: is casting ok?
...
def contextual_attention_layer(
    ...
    with tf.compat.v1.variable_scope(
        ...
        hidden_size = smiles.shape[2]
        num_genes = genes.shape[1]
        num_gene_features = genes.shape[2]
        ...
        b_genes = tf.Variable(tf.random.normal([attention_size], stddev=0.1))

        
        w_smiles = tf.Variable(
            tf.random.normal([hidden_size, attention_size], stddev=0.1)
        )
        b_smiles = tf.Variable(tf.random.normal([attention_size], stddev=0.1))
        v = tf.Variable(tf.random.normal([attention_size], stddev=0.1))
